"""Middleware for setting client-side Cache-Control headers in FastAPI responses.

This module provides ClientCacheMiddleware, which adds a configurable Cache-Control header
to all HTTP responses for client-side caching.
"""
...
class ClientCacheMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: FastAPI, max_age: int = 60) -> None:
        """Initialize the middleware with the given FastAPI application and cache max age.

        Args:
            app (FastAPI): The FastAPI application instance to wrap with the middleware.
            max_age (int, optional): The maximum age (in seconds) for client-side caching. Defaults to 60.
